...
if ! grep -Fxq "$checkConfig" /etc/environment; then
    sudo -i <<-EOF
    echo -e "# Proxy ad.harderwijk.local
    http_proxy=\"http:\/\/10.0.0.14:3129\/\"
    https_proxy=\"http:\/\/10.0.0.14:3129\/\"
    ftp_proxy=\"http:\/\/10.0.0.14:3129\/\"
    no_proxy=\"localhost,127.0.0.1,localaddress,.localdomain.com\/\"
    HTTP_PROXY=\"10.0.0.14:3129\/\"
    HTTPS_PROXY=\"10.0.0.14:3129\/\"
    FTP_PROXY=\"10.0.0.14:3129\/\"
